...
ShipState SimpleNomotoShipModel::getState(double sim_time_s) const {
    ...
    return ShipState {
        ...
        lat_deg,
        lon_deg,
        math::radToDeg(heading_rad_),
        speed_mps_,
        math::radToDeg(yaw_rate_rps_)
    };
}
...
void SimpleNomotoShipModel::updateYawRate(double dt_s) {
    const double r_dot = ((config_.nomoto_K * rudder_cmd_rad_) - yaw_rate_rps_) / config_.nomoto_T_s;
...
std::pair<double, double> SimpleNomotoShipModel::toLatLonDeg() const {
    return math::localOffsetToLatLonDeg(
        lat0_deg_,
        lon0_deg_,
        x_m_,
        y_m_,
        config_.earth_radius_m);
}
